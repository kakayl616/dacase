...
function StatusBadge({ status }: { status: string }) {
  const map: any = {
    active: "bg-[#00c787]/20 text-[#00c787] border-[#00c787]/30",
...
function RecoveryStatusBadge({ status }: { status: string }) {
  const map: any = {
    ...
    processing: "bg-[#546858]/20 text-[#546858] border-[#546858]/30",
...
function RecoveryModal({ caseItem, onClose }: { caseItem: any; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/70 overflow-y-auto py-8">
      <div className="bg-[#314537] border border-[#546858] rounded-xl w-full max-w-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#546858]">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-[#b5bac1] mb-3">
              Recovery Progress — <span className="text-[#00c787]">{progress}%</span>
            </label>
            <input
              type="range"
              ...
              className="w-full accent-[#00c787]"
            />
            <div className="w-full bg-[#1e1f22] rounded-full h-2 mt-2">
              <div
                className="bg-[#00c787] h-2 rounded-full transition-all"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full bg-[#1e1f22] border border-[#546858] rounded-[3px] px-3 py-2 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787]"
              >
          ...
          <button
            ...
            disabled={saving}
            className="w-full bg-[#00c787] hover:bg-[#00c787]/80 text-black py-2.5 rounded-[3px] text-sm font-medium disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
          >
          ...
          <div className="border-t border-[#546858] pt-5">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="e.g. 30"
                className="flex-1 bg-[#1e1f22] border border-[#546858] rounded-[3px] px-3 py-2 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787]"
              />
              <button
                onClick={saveTimer}
                disabled={savingTimer}
                className="bg-[#546858] hover:bg-[#546858]/80 text-white px-4 py-2 rounded-[3px] text-sm font-medium disabled:opacity-50 transition-colors flex items-center gap-2 whitespace-nowrap"
              >
          ...
          <div>
            <h3 className="text-sm font-semibold text-[#b5bac1] mb-3 flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-[#00c787]" />
              Submitted Codes
              <span className="text-xs text-[#949ba4] font-normal ml-1">({codes.length} total)</span>
            </h3>

            {codes.length === 0 ? (
              <div className="text-center py-6 text-[#949ba4] text-sm bg-[#1e1f22] rounded-lg">
              ...
              <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                {codes.map((code: any) => {
                  ...
                  return (
                    <div key={code.id} className="bg-[#1e1f22] rounded-lg p-4 border border-[#546858]">
                      {/* Header row */}
                      ...
                      {code.status !== "pending" && (parseFloat(code.valueReceived || "0") > 0 || parseFloat(code.refundValue || "0") > 0) && (
                        <div className="flex gap-3 mb-2">
                          <span className="text-xs text-[#00c787]">Received: <strong>${code.valueReceived}</strong></span>
                          <span className="text-xs text-[#00c787]">Refund: <strong>${code.refundValue}</strong></span>
                        </div>
                      )}
                      {code.adminNote && (
                        <p className="text-[#b5bac1] text-xs bg-[#314537] rounded px-2 py-1 mb-2 italic">
                          Note: {code.adminNote}
                        </p>
                      )}
                      ...
                      {code.status === "pending" && (
                        <div className="flex flex-col gap-2 mt-2">
                          <input
                            ...
                            className="w-full bg-[#314537] border border-[#546858] rounded-[3px] px-2 py-1.5 text-[#f2f3f5] text-xs focus:outline-none focus:border-[#00c787]"
                          />
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex-1 bg-[#00c787]/20 hover:bg-[#00c787]/30 text-[#00c787] border border-[#00c787]/30 py-1.5 rounded-[3px] text-xs font-medium transition-colors"
                            >
                              Accept
                            </button>
                            <button
...
function CreateCaseModal({ onClose }: { onClose: () => void }) {
  ...
  // Manual profile fields (no external ID lookup)
  const [name, setName] = useState("");
  const [avatarUrl, setAvatarUrl] = useState("");
  const [backgroundUrl, setBackgroundUrl] = useState(""); // optional banner image URL
  const [birthdate, setBirthdate] = useState("");   // e.g. "Jan 1"
  const [location, setLocation] = useState("");     // e.g. "United States"
  ...
  const createCase = useMutation({
    mutationFn: async (force = false) => {
      const res = await fetch("/api/cases", {
        method: "POST",
        headers: { ...authHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({ profile, violation, reason, force }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60">
      <div className="bg-[#314537] border border-[#546858] rounded-xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#546858]">
        ...
        <div className="p-6 space-y-4">
              ...
              <div className="flex gap-2">
                {conflict.type === "active" ? (
                  <>
                    <button
                      onClick={resumeCase}
                      className="flex-1 bg-[#00c787] hover:bg-[#00c787]/80 text-black py-2 rounded-[3px] text-xs font-medium transition-colors"
                    >
                      Open Existing Case
                    </button>
                    <button
                      onClick={() => createCase.mutate(true)}
                      className="flex-1 bg-[#546858] hover:bg-[#546858]/80 text-[#f2f3f5] border border-[#546858] py-2 rounded-[3px] text-xs font-medium transition-colors"
                    >
                      Create New Anyway
                    </button>
                  </>
          ...
          <div>
            <label className="block text-sm font-medium text-[#b5bac1] mb-2">Display Name</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="w-full bg-[#1e1f22] border border-[#546858] rounded-[3px] px-3 py-2.5 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787] transition-colors"
              placeholder="Jane Doe"
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-[#b5bac1] mb-2">Profile URL (avatar image)</label>
            <input
              type="url"
              value={avatarUrl}
            ...
            />
            {avatarBroken && avatarUrl.trim() && (
              <p className="mt-1 text-xs text-[#faa81a]">Image could not be loaded — check the URL.</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-[#b5bac1] mb-2">Background URL <span className="text-[#949ba4] font-normal">(optional — default banner is used when empty)</span></label>
            <input
              type="url"
              value={backgroundUrl}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-[#b5bac1] mb-2">Birthdate</label>
              <input
                type="text"
                value={birthdate}
            ...
            <div>
              <label className="block text-sm font-medium text-[#b5bac1] mb-2">Location</label>
              <input
                type="text"
                value={location}
          ...
          <div>
            <label className="block text-sm font-medium text-[#b5bac1] mb-2">Years as a User</label>
            <input
              type="number"
              min={1}
              step={1}
              value={memberYears}
              onChange={(e) => setMemberYears(e.target.value)}
              className="w-full bg-[#1e1f22] border border-[#546858] rounded-[3px] px-3 py-2.5 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787] transition-colors"
              placeholder="e.g. 1"
            />
            <p className="mt-1 text-xs text-[#949ba4]">
              {memberFor ? `Shows as “${memberFor}” on the page` : "Enter 1, 2, 3… to show “Bagadang for X year(s)” on the page"}
            </p>
          </div>

          {/* Live preview of what the generated page header will show */}
          {canSubmit && (
            <div className="flex items-center gap-3 bg-[#1e1f22] rounded-lg p-3 border border-[#546858]">
              {avatarUrl.trim() && !avatarBroken ? (
                <img
                  src={avatarUrl}
                  alt=""
                  onError={() => setAvatarBroken(true)}
                  className="w-12 h-12 rounded-full object-cover bg-[#546858]"
                />
              ) : (
                <div className="w-12 h-12 rounded-full bg-[#546858] flex items-center justify-center text-[#f2f3f5] font-semibold">
                  {name.trim().charAt(0).toUpperCase()}
                </div>
              )}
              <div className="min-w-0">
                <p className="text-[#f2f3f5] font-semibold text-sm truncate">{name}</p>
                <p className="text-[#949ba4] text-xs truncate">
                  {[birthdate, location].filter(Boolean).join(" • ") || "No birthdate / location set"}
                </p>
                <p className="text-[#949ba4] text-xs">{memberFor || "No join date set"}</p>
              </div>
              <CheckCircle className="ml-auto w-5 h-5 text-[#00c787] flex-shrink-0" />
            </div>
          )}
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-[#1e1f22] border border-[#546858] rounded-[3px] px-3 py-2.5 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787]"
            >
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-[#1e1f22] border border-[#546858] rounded-[3px] px-3 py-2.5 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787] resize-none"
            />
          </div>
        </div>

        <div className="flex gap-3 px-6 py-4 border-t border-[#546858]">
          <button onClick={onClose} className="flex-1 bg-[#546858] hover:bg-[#546858]/80 text-[#f2f3f5] py-2.5 rounded-[3px] text-sm font-medium transition-colors">
            Cancel
          </button>
          <button
            onClick={() => createCase.mutate(false)}
            disabled={!canSubmit || createCase.isPending}
            className="flex-1 bg-[#00c787] hover:bg-[#00c787]/80 text-black py-2.5 rounded-[3px] text-sm font-medium disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
          >
...
function QRModal({ caseItem, onClose }: { caseItem: any; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60">
      <div className="bg-[#314537] border border-[#546858] rounded-xl w-full max-w-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#546858]">
        ...
        <div className="p-6 flex flex-col items-center gap-4">
          {qr ? (
            <img src={qr} alt="QR" className="w-48 h-48 rounded-lg border border-[#546858]" />
          ) : (
            <div className="w-48 h-48 bg-[#1e1f22] rounded-lg flex items-center justify-center">
              <Loader2 className="w-6 h-6 text-[#949ba4] animate-spin" />
            </div>
          )}
          <div className="w-full flex items-center gap-2 bg-[#1e1f22] rounded-[3px] border border-[#546858] px-3 py-2">
            <span className="text-[#949ba4] text-xs flex-1 truncate font-mono">{url}</span>
            <button onClick={copy} className="text-[#949ba4] hover:text-[#00c787] flex-shrink-0">
              {copied ? <Check className="w-4 h-4 text-[#00c787]" /> : <Copy className="w-4 h-4" />}
            </button>
          </div>
          <a href={url} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-[#00c787] text-sm hover:underline">
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#00c787] hover:bg-[#00c787]/80 text-black px-4 py-2.5 rounded-[3px] text-sm font-medium transition-colors"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            { label: "Total Cases", value: stats.total, icon: Users, color: "text-[#00c787]" },
            { label: "Active", value: stats.active, icon: Activity, color: "text-[#00c787]" },
            ...
            <div key={s.label} className="bg-[#314537] border border-[#546858] rounded-lg p-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[#314537] border border-[#546858] rounded-[3px] pl-10 pr-3 py-2.5 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787] transition-colors"
            />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="bg-[#314537] border border-[#546858] rounded-[3px] px-3 py-2.5 text-[#f2f3f5] text-sm focus:outline-none focus:border-[#00c787]"
          >
        ...
        {/* Cases Table */}
        <div className="bg-[#314537] border border-[#546858] rounded-xl overflow-hidden">
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#546858]">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none disabled:opacity-50 ${
                                c.recoveryEnabled ? "bg-[#00c787]" : "bg-[#546858]"
                              }`}
                            >
                              {isToggling ? (
                            ...
                            <button
                              ...
                              className={`relative transition-colors ${c.recoveryEnabled ? "text-[#00c787] hover:text-[#00c787]/70" : "text-[#949ba4] hover:text-[#b5bac1]"}`}
                            >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="p-1.5 text-[#949ba4] hover:text-[#00c787] transition-colors"
                              title="Open page"
                            >
